...
const Form = ({ name, fields, submit, provider }) => {
  ...
  useEffect(() => {
    ...
    const loadSelectOptions = async () => {
      ...
      for (const field of fields) {
        if (field.type === "select" && field.select_options?.type === "dynamic") {
          try {
            const { data } = await dataProvider.getAll(field.select_options.provider);
            optionsMap[field.field] = data.map(item => ({
              value: item[field.select_options.value_field],
              label: item[field.select_options.label_field]
            }));
          } catch (error) {
  ...
  const renderField = (field, index) => {
    if (field.type === "select") {
      const options = field.select_options?.type === "dynamic" 
